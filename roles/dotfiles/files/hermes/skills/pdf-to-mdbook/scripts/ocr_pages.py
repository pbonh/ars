...
# Below this many characters, treat Tesseract output as a failure and
# fall back to embedding the rasterized page as a figure. Heavily styled
# pages (decorative resumes, infographics, badly scanned scans) are the
# usual culprits; without a fallback they silently disappear from the
# assembled book.
OCR_MIN_CHARS = 30


def _image_fallback_markdown(page: int, char_count: int,
                             rel_image_path: str) -> str:
    return (
        f"<!-- ocr-fallback: tesseract produced {char_count} chars on this page; "
        f"embedding rendered image instead -->\n"
        f"![Page {page} (rendered)]({rel_image_path})\n"
    )
...
def ocr_one_page(pdf: str, page: int, dpi: int, lang: str, psm: int,
                 preprocess: str | None,
                 out_dir: Path) -> tuple[str, bool]:
    """Rasterize page (1-indexed) and OCR it.

    Returns ``(markdown, used_fallback)``. ``used_fallback`` is True when
    Tesseract returned fewer than ``OCR_MIN_CHARS`` characters and we
    persisted the rasterized page as an image instead.
    """
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            return f"<!-- pdftoppm failed for page {page}: {e.stderr.decode()} -->", False

        # Find the produced image (zero-padded by pdftoppm based on page count)
        produced = sorted(Path(tmp).glob("p-*.png"))
        if not produced:
            return f"<!-- no image produced for page {page} -->", False
        ...
        try:
            ...
            text = ""
            stderr_msg = e.stderr if isinstance(e.stderr, str) else e.stderr.decode(errors="replace")
            failure_note = f"<!-- tesseract failed for page {page}: {stderr_msg.strip()} -->\n"
        else:
            failure_note = ""

        # Fallback: not enough recognizable text → save the rendered page
        # as an image and return a Markdown image reference. This keeps
        # heavily-stylized pages from disappearing from the book.
        real_chars = len(text.strip())
        if real_chars < OCR_MIN_CHARS:
            images_dir = out_dir / "images"
            images_dir.mkdir(parents=True, exist_ok=True)
            dest = images_dir / f"page_{page:04d}.png"
            shutil.copyfile(img_path, dest)
            return failure_note + _image_fallback_markdown(
                page, real_chars, f"images/{dest.name}"
            ), True
...
def main():
    ...
    for p in target_pages:
        ...
        if fname.exists() and not args.force:
            ...
            used_fallback = "ocr-fallback:" in text[:200]
        else:
            text, used_fallback = ocr_one_page(
                args.pdf, p, args.dpi, args.lang,
                args.psm, preprocess, out_dir,
            )
            ...
            if done % 5 == 0 or p == target_pages[-1]:
                marker = " [image-fallback]" if used_fallback else ""
                print(f"  ocr {done}/{len(target_pages) - skipped}: "
                      f"page {p} → {len(text)} chars{marker}",
                      file=sys.stderr)
    ...
    if fallback_count:
        print(f"  image-fallback used on {fallback_count} page(s) "
              f"(< {OCR_MIN_CHARS} chars from tesseract → embedded as image). "
              f"PNGs in {out_dir / 'images'}.")
