...
def dehyphenate(text: str) -> str:
    ...
    proper noun)."""
    # Join hyphen-newline-lowercase across line breaks
    text = re.sub(r"(\w)-\n([a-z])", r"\1\2", text)
    # Hyphen-blankline-lowercase across page breaks (blank lines remain
    # from page concatenation): treat similarly
    text = re.sub(r"(\w)-\n\s*\n([a-z])", r"\1\2", text)
    return text
...
def strip_repeated_lines(pages_text: list[str], frac: float = 0.6) -> list[str]:
...
def assemble(outline: list[dict], pages_dir: Path, manifest: dict,
    ...
    for i, item in enumerate(items_by_page):
        start = item["page"]
        # Chapter ends at the start of the next *level <= current level* item
        end = last_page
        for j in range(i + 1, len(items_by_page)):
            if items_by_page[j]["level"] <= item["level"]:
                end = items_by_page[j]["page"] - 1
                break
...
def main():
    ...
    ap.add_argument("--header-strip-frac", type=float, default=0.6,
                    help="Fraction of pages on which a top/bottom line must "
                         "appear to be stripped (default: 0.6)")
    args = ap.parse_args()
