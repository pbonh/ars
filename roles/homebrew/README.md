...
2. Run `brew bundle check --file …` (if enabled) then `brew bundle install --file …` with the resolved PATH.
3. Run `brew bundle cleanup --force --file …` when cleanup is enabled.
...
- Cleanup: `brew bundle cleanup --file ~/.Brewfile --force`

Note
----
`Brewfile.lock.json` is generated by `brew bundle` and can be managed separately if desired.
